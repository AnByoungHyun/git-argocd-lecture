...
image: node-app:abc1234"]
    K8s["☸️ K8s 클러스터
현재 실행 중인 Deployment
image: node-app:xyz9012"]
    Diff{"🔍 diff
다름!"}
    Out["⚠️ OutOfSync"]

    Git <-->|"ArgoCD 주기적 폴링 3분"| K8s
    Git --> Diff
    K8s --> Diff
    Diff -->|"불일치 감지"| Out
...
stateDiagram-v2
    [*] --> Synced : 초기 배포 완료
    Synced --> OutOfSync : Git에 새 커밋 발생 (이미지 태그 변경)
    OutOfSync --> Progressing : auto-sync 활성화 (ArgoCD kubectl apply)
    Progressing --> Synced : readinessProbe 통과 (새 Pod Ready → 구 Pod 종료)
    Progressing --> Degraded : Pod 기동 실패 (이미지 오류 등)
    Degraded --> OutOfSync : 수동 수정 또는 rollback 후
```
...
sequenceDiagram
    ...
    Op->>K8s: kubectl scale deployment node-app --replicas=5
    K8s-->>Argo: 클러스터 상태 변경 감지 (replicas=5)
    Argo->>Git: Git 상태 조회 (replicas=1)
    Note over Argo: 불일치 감지! selfHeal: true → 자동 복원
    Argo->>K8s: kubectl apply -f deployment.yaml (replicas=1 강제 복원)
    K8s-->>Argo: 약 30초 이내 복원 완료 ✅
```
...
✅ 팀원 공유 가능"]
    ...
    subgraph "⚠️ 방법 B: ArgoCD UI 롤백 (긴급 시만)"
        B1["ArgoCD UI
History → Rollback"]
        B2["클러스터: 이전 이미지 즉시 복원"]
        B3["⚠️ Git 미반영
⚠️ 다음 Sync 시 최신 Git으로 덮어쓰임
⚠️ 이력 추적 어려움"]
        B4["반드시 git revert로 Git도 맞출 것"]
        B1 --> B2 --> B3 --> B4
    end
...
sequenceDiagram
    ...
    Dev->>GH: ① git push main (app/node-app/ 변경)
    GH->>CI: ② ci-node.yml 트리거 (path 필터 매칭)
    CI->>CI: Job1: npm ci + npm test ✅
    CI->>GHCR: Job2: docker push node-app:SHA + latest
    CI->>GH: ③ manifests/ 이미지 태그 업데이트 커밋 (image:node-app:NEW-SHA)

    loop 3분 주기 Git 폴링
        Argo->>GH: ④ 변경 감지 (OutOfSync)
    end

    Argo->>K8s: auto-sync → kubectl apply
    K8s->>GHCR: 새 이미지 Pull (node-app:NEW-SHA)
    K8s->>K8s: ⑤ RollingUpdate (신규 Pod 기동 → readinessProbe 통과 → 구 Pod 종료)
    K8s-->>Argo: ⑥ Synced + Healthy ✅
    K8s-->>Dev: curl apps.local/node → 새 버전 응답
```
...
git commit + push main"]
    B["② GitHub Actions CI 트리거
app/node-app/** → ci-node.yml"]
    C["Job1: build-and-test
checkout → npm ci → npm test"]
    D{"테스트 통과?"}
    E["Job2: docker-build-push
docker buildx build
GHCR push: SHA + latest ✅"]
    F["실패 ❌
CI 중단, GHCR 변경 없음"]
    G["③ 매니페스트 업데이트
manifests/node-app/deployment.yaml
...
git commit + push"]
    H["④ ArgoCD 감지
3분 주기 Git 폴링
OutOfSync 감지 → auto-sync"]
    I["⑤ K8s RollingUpdate
신규 Pod 기동 → readinessProbe 통과
구 Pod 종료 무중단"]
    J["⑥ 배포 완료 ✅
Synced + Healthy
curl apps.local/node → 새 버전"]
